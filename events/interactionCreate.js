// ============================================================================
// MASTER INTERACTION HANDLER - BUDDYBOT RCE (FULL & TIMEOUT SHIELDED)
// ============================================================================
...
// Safety helper to prevent database hangs from freezing Discord
async function safeDbQuery(queryFn, fallback = null) {
    try {
        ...
        console.warn('[DB WARNING] Database query skipped or timed out:', e.message);
        return fallback;
    }
}
    ...
    try {
        ...
        if (interaction.isRoleSelectMenu()) {
            if (interaction.customId.startsWith('bind_role_menu_')) {
                ...
                await safeDbQuery(() => CustomBind.update({ roleId: interaction.values[0] }, { where: { id: bindId } }));
                return interaction.update({ content: `✅ Bind finalized!`, components: [] });
            }
            if (interaction.customId === 'select_ticket_admin') {
                await safeDbQuery(() => GuildConfig.upsert({ guildId: interaction.guild.id, ticketAdminRoleId: interaction.values[0] }));
                return interaction.update({ content: `✅ Ticket Admin Role set!`, components: [] });
            }
            if (interaction.customId === 'select_ticket_vip') {
                await safeDbQuery(() => GuildConfig.upsert({ guildId: interaction.guild.id, ticketVipRoleId: interaction.values[0] }));
            ...
            if (interaction.customId.startsWith('shop_role_')) {
                ...
                await safeDbQuery(() => ShopItem.update({ requiredRoleId: interaction.values[0] }, { where: { id: itemId } }));
            ...
            if (interaction.customId === 'select_suggestion_role') {
                await safeDbQuery(() => GuildConfig.upsert({ guildId: interaction.guild.id, suggestionPingRoleId: interaction.values[0] }));
        ...
        if (interaction.isChannelSelectMenu()) {
            if (interaction.customId === 'select_crosschat_channel') {
                await safeDbQuery(() => GuildConfig.upsert({ guildId: interaction.guild.id, crossChatChannelId: interaction.values[0] }));
                return interaction.update({ content: `✅ Cross-Chat linked!`, components: [] });
            }
            if (interaction.customId === 'select_killfeed_channel') {
                await safeDbQuery(() => GuildConfig.upsert({ guildId: interaction.guild.id, killfeedChannelId: interaction.values[0] }));
            ...
            if (interaction.customId === 'select_giveaway_channel') {
                await safeDbQuery(() => GuildConfig.upsert({ guildId: interaction.guild.id, giveawayChannelId: interaction.values[0] }));
            ...
            if (interaction.customId === 'select_ticket_category') {
                await safeDbQuery(() => GuildConfig.upsert({ guildId: interaction.guild.id, ticketCategoryId: interaction.values[0] }));
                return interaction.update({ content: `✅ Ticket Category set!`, components: [] });
            }
            if (interaction.customId === 'select_ticket_transcript') {
                await safeDbQuery(() => GuildConfig.upsert({ guildId: interaction.guild.id, ticketTranscriptChannelId: interaction.values[0] }));
            ...
            if (interaction.customId === 'select_log_admin_channel') {
                await safeDbQuery(() => GuildConfig.upsert({ guildId: interaction.guild.id, logAdminChannelId: interaction.values[0] }));
            ...
            if (interaction.customId === 'select_log_game_channel') {
                await safeDbQuery(() => GuildConfig.upsert({ guildId: interaction.guild.id, logGameChannelId: interaction.values[0] }));
            ...
            if (interaction.customId === 'select_log_discord_channel') {
                await safeDbQuery(() => GuildConfig.upsert({ guildId: interaction.guild.id, logDiscordChannelId: interaction.values[0] }));
            ...
            if (interaction.customId === 'select_suggestion_channel') {
                await safeDbQuery(() => GuildConfig.upsert({ guildId: interaction.guild.id, suggestionChannelId: interaction.values[0] }));
        ...
        if (interaction.isUserSelectMenu()) {
            if (interaction.customId === 'select_give_item_target') {
                const targetUser = await safeDbQuery(() => UserEconomy.findOne({ where: { guildId: interaction.guild.id, userId: interaction.values[0] } }));
            ...
            if (interaction.customId.startsWith('select_ga_reroll_')) {
                ...
                const ga = await safeDbQuery(() => Giveaway.findByPk(messageId));
            ...
            if (interaction.customId === 'select_admin_give_target') {
                ...
                const targetUser = await safeDbQuery(() => UserEconomy.findOne({ where: { guildId: interaction.guild.id, userId: targetUserId } }));
        ...
        if (interaction.isStringSelectMenu()) {
            ...
            if (interaction.customId === 'select_pve_delete_exec') {
                ...
                const zoneId = module;
                const zone = await safeDbQuery(() => PveZone.findByPk(zoneId));
            ...
            if (interaction.customId === 'hub_lb_select') {
                ...
                const config = await safeDbQuery(() => GuildConfig.findOne({ where: { guildId } }));
                const currency = config ? config.economyCurrency : 'Scrap';
                const allPlayers = await safeDbQuery(() => UserEconomy.findAll({ where: { guildId } }), []);
                let leaderboardText = '';
                let embedTitle = '';
                let embedColor = '';
            ...
            if (interaction.customId === 'admin_menu_select') {
                ...
                if (module === 'setup_clans') {
                    const config = await safeDbQuery(() => GuildConfig.findOne({ where: { guildId: interaction.guild.id } }), {});
                    const activeClans = await safeDbQuery(() => Clan.count({ where: { guildId: interaction.guild.id } }), 0);
                ...
                if (module === 'setup_bounties') {
                    const config = await safeDbQuery(() => GuildConfig.findOne({ where: { guildId: interaction.guild.id } }), {});
                    const activeBounties = await safeDbQuery(() => ActiveBounty.count({ where: { guildId: interaction.guild.id } }), 0);
                ...
                if (module === 'setup_buddypass') {
                    const config = await safeDbQuery(() => GuildConfig.findOne({ where: { guildId: interaction.guild.id } }), {});
                    const challenges = await safeDbQuery(() => BuddyPassChallenge.findAll({ where: { guildId: interaction.guild.id } }), []);
                    const rewards = await safeDbQuery(() => BuddyPassReward.findAll({ where: { guildId: interaction.guild.id } }), []);
                ...
                if (module === 'setup_orp') {
                    const orpConfig = await safeDbQuery(() => OrpConfig.findOne({ where: { guildId: interaction.guild.id } }), {});
                    const embed = new EmbedBuilder()
                ...
                if (module === 'setup_pvezones') {
                    const zones = await safeDbQuery(() => PveZone.findAll({ where: { guildId: interaction.guild.id } }), []);
                ...
                if (module === 'setup_tickets') {
                    const cats = await safeDbQuery(() => TicketCategory.findAll({ where: { guildId: interaction.guild.id } }), []);
            ...
            if (interaction.customId === 'select_ai_provider') {
                ...
                let [config] = await safeDbQuery(() => GuildConfig.findOrCreate({ where: { guildId: interaction.guild.id } }), [{}]);
            ...
            if (interaction.customId === 'shop_action_select') {
                ...
                if (module === 'shop_manage') {
                    const items = await safeDbQuery(() => ShopItem.findAll({ where: { guildId: interaction.guild.id } }), []);
            ...
            if (interaction.customId === 'shop_catalog_multi_select') {
                ...
                for (const val of checkedItems) {
                    ...
                    if (catalogItem) {
                        await safeDbQuery(() => ShopItem.create({
            ...
            if (interaction.customId === 'ticket_action_select') {
                ...
                if (module === 'tk_toggle') {
                    ...
                    await safeDbQuery(() => GuildConfig.upsert({ guildId: interaction.guild.id, ticketSendUserTranscript: newState }));
            ...
            if (interaction.customId === 'player_shop_cat_select') {
                const catKey = module;
                const categoryData = RUST_CATEGORIES[catKey];
                const dbItems = await safeDbQuery(() => ShopItem.findAll({ where: { guildId: interaction.guild.id } }), []);
                const config = await safeDbQuery(() => GuildConfig.findOne({ where: { guildId: interaction.guild.id } }), {});
            ...
            if (interaction.customId === 'player_shop_buy_select') {
                ...
                const shopItem = await safeDbQuery(() => ShopItem.findByPk(itemId));
            ...
            if (interaction.customId === 'bind_template_select') {
                ...
                if (template === 'tpl_kit') {
                    const kits = await safeDbQuery(() => ServerKit.findAll({ where: { guildId: interaction.guild.id } }), []);
                    ...
                    const userProfile = await safeDbQuery(() => UserEconomy.findOne({ where: { guildId: interaction.guild.id, userId: interaction.user.id } }));
        ...
        if (interaction.isButton()) {
            if (interaction.customId === 'hub_clans') {
                ...
                const config = await safeDbQuery(() => GuildConfig.findOne({ where: { guildId: interaction.guild.id } }), {});
                const memberData = await safeDbQuery(() => ClanMember.findOne({ where: { guildId: interaction.guild.id, userId: interaction.user.id } }));

                if (!memberData) {
                    const invites = await safeDbQuery(() => ClanInvite.count({ where: { guildId: interaction.guild.id, userId: interaction.user.id } }), 0);
                    const embed = new EmbedBuilder().setTitle('🛡️ Clan System').setDescription(`You are currently not in a clan.\n\nCreate your own clan for **${config?.clanCreationCost || 1000} ${config?.economyCurrency || 'Scrap'}** or check invites.`).setColor('#3498db');
                    const row = new ActionRowBuilder().addComponents(
                    ...
                    const clan = await safeDbQuery(() => Clan.findByPk(memberData.clanId));
                    const embed = new EmbedBuilder().setTitle(`🛡️ ${clan?.tag || ''} ${clan?.name || 'Clan'}`).setDescription(`**Your Role:** ${memberData.role}\n**Bank:** ${clan?.bankBalance || 0}`).setColor('#f1c40f');
                    return interaction.reply({ embeds: [embed], flags: 64 });
                }
            }

            if (interaction.customId === 'hub_bounties') {
                const bounties = await safeDbQuery(() => ActiveBounty.findAll({ where: { guildId: interaction.guild.id } }), []);
                if (bounties.length === 0) return interaction.reply({ content: '🎯 No active bounties currently on the server.', flags: 64 });
                const list = bounties.map(b => `🎯 **${b.inGameName}** — Reward: **${b.reward} Scrap**`).join('\n');
                const embed = new EmbedBuilder().setTitle('🎯 Active Player Bounties').setDescription(list).setColor('#e74c3c');
                return interaction.reply({ embeds: [embed], flags: 64 });
            }

            if (interaction.customId === 'hub_suggestion') {
                const config = await safeDbQuery(() => GuildConfig.findOne({ where: { guildId: interaction.guild.id } }), {});
                if (!config?.suggestionChannelId) return interaction.reply({ content: '❌ Suggestions system not configured by an admin yet.', flags: 64 });
            ...
            if (interaction.customId === 'btn_ae_test_cargo') {
                ...
                if (config && config.cargoDockX !== null && config.cargoDockY !== null && config.cargoDockZ !== null) {
                    ...
                    return interaction.editReply({ content: `✅ Spawned Docked Cargo Ship successfully!` });
                } else {
                    await sendRconCommand(interaction.guild.id, 'cargoships.spawncargoship');
                    return interaction.editReply({ content: `⚠️ Triggered standard roaming cargo event test!` });
                }
            }
            if (interaction.customId === 'btn_ae_test_supply') {
                await interaction.deferReply({ flags: 64 });
                await sendRconCommand(interaction.guild.id, 'supply.drop');
                return interaction.editReply({ content: `✅ Test supply drop triggered!` });
            }
        }
        ...
        if (interaction.isModalSubmit()) {
            if (interaction.customId === 'modal_hub_suggestion') {
                ...
                const embed = new EmbedBuilder().setTitle(`💡 Suggestion: ${title}`).setDescription(desc).setColor('#f1c40f').setTimestamp();
                ...
                const row = new ActionRowBuilder().addComponents(
                    new StringSelectMenuBuilder().setCustomId('casino_game_select').setPlaceholder('Select minigame...')
                        .addOptions([
                            ...
                            { label: 'Dice Roll', value: 'dice', emoji: '🎲' }
                        ])
                );
